...
orientation: {
  ...
  roll : 0.0 // No roll
          ...
          roll : 0.0 // No roll
            ...
            destination: Cesium.Cartesian3.fromDegrees(-118.216, 33.749, 900), // Adjusted for close-up view
            orientation: {
            heading : Cesium.Math.toRadians(10.0), // North
...
"</ul>",
         destination: Cesium.Cartesian3.fromDegrees(-118.2500, 33.74546, 163734.072), // Approximate location over the port
         orientation: {
...
"</ul>",            
     destination: Cesium.Cartesian3.fromDegrees(-118.2370, 33.7380, 2500), // This brings the camera closer to the ground
     orientation: {
...
"<p><li>Nitrogen Oxides (NOx) by 62% </p></li>" + 
        "</ul>", 
           destination: Cesium.Cartesian3.fromDegrees(-118.1480, 33.7780, 3800), // These coordinates position the camera above the port area.
           orientation: {
        heading: Cesium.Math.toRadians(15), // This heading rotates the camera to an angle that approximates the northwest direction.
        pitch: Cesium.Math.toRadians(-30), // This pitch tilts the camera towards the ground at a diagonal angle.
...
"<p>In its continued efforts to modernize, the Port has implemented shore power at several terminals, enabling ships to connect to the electrical grid while docked, thereby substantially reducing auxiliary engine use. Concurrently, the transition to zero-emissions cargo handling equipment is in progress, marked by the introduction of electric yard trucks and forklifts. Exploring hydrogen fuel cells as an alternative power source for cargo handling equipment and ships, and offering incentives for using clean energy and green technologies to vessels and trucking companies, are also a key part of the Port's forward-thinking strategy.</p>",  
       destination: Cesium.Cartesian3.fromDegrees(-118.166, 33.549, 800), // This sets the camera above the port area with containers in view.
orientation: {
  heading: Cesium.Math.toRadians(30), // Heading at 0 degrees for north.
...
"</ul>", 
destination: Cesium.Cartesian3.fromDegrees(-118.210, 33.749, 3500), // This sets the camera above the area of interest.
orientation: {
...
destination: Cesium.Cartesian3.fromDegrees(-118.210, 33.730, 5550), // Adjust the coordinates and height to get the right angle and zoom level.
orientation: {
...
"<p><li>Educational and Workforce Development: Raises awareness about port operations and offers training programs, strengthening the local workforce.</li></p>" + 
                "</ul>",
destination: Cesium.Cartesian3.fromDegrees(-118.191, 33.726, 3600), // Adjusted coordinates for the desired view
orientation: {
...
"</ul>", 
destination: Cesium.Cartesian3.fromDegrees(-118.225, 33.720, 8000), // Coordinates over the Port of Long Beach
orientation: {
  ...
  roll: 0.0  // Level horizon
            ...
            destination: Cesium.Cartesian3.fromDegrees(-118.3565, 33.7450, 12500),
          orientation: {
